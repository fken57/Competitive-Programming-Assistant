...
import { buildPrimVisualGraphData } from '../../../../util/WeightedResultGraphTransforms';
...
export function Prim({ adjacentList }: Props) {
    ...
    const resultVisualData = useMemo(
        () => buildPrimVisualGraphData(adjacentList, data),
        [adjacentList, data]
    );
                    ...
                    <div className="bfs-visualizer-wrapper">
                        <div className="bfs-visualizer-box">
                            <GraphVisualizer
                                ...
                                edgeColorFn={(edge) => edge.attributes?.inSpanningForest ? '#2563EB' : '#D1D5DB'}
                                edgeWidthFn={(edge) => edge.attributes?.inSpanningForest ? 5 : 1.5}
                            />
                        </div>
                    </div>
                )}
