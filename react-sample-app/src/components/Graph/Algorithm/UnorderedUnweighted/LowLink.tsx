...
import { buildLowLinkVisualGraphData } from '../../../../util/LowLinkGraphJsonTransform';
...
export function LowLink({ adjacentList }: Props) {
    ...
    const resultVisualData = useMemo(
        () => buildLowLinkVisualGraphData(adjacentList, data),
        [adjacentList, data]
    );
                    ...
                    <div className="bfs-visualizer-wrapper">
                        <div className="bfs-visualizer-box">
                            <GraphVisualizer
                                ...
                                nodeColorFn={(node) => node.attributes?.isArticulationPoint ? '#FDE68A' : '#FFFFFF'}
                                nodeStrokeColorFn={(node) => node.attributes?.isArticulationPoint ? '#EA580C' : '#666'}
                                nodeStrokeWidthFn={(node) => node.attributes?.isArticulationPoint ? 5 : 2}
                                edgeColorFn={(edge) => edge.attributes?.isBridge ? '#DC2626' : '#9CA3AF'}
                                edgeWidthFn={(edge) => edge.attributes?.isBridge ? 5 : 2}
                            />
                        </div>
                    </div>
                )}
